...
const pharmacyServiceSvcInstance = new PharmacyService();
...
app.post('/pharmacyReceived', async (req, res, next) => {
  ...
  try {
    if(!userName || userName.lenth<1) {
      ...
      const result = await await pharmacyServiceSvcInstance.pharmacyReceived(userName, equipmentNumber, ownerName);
